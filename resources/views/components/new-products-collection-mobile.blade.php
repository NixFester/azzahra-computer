            ...
            <div class="category-scroll-container">
                <ul class="nav nav-pills category-pills gap-2" id="productCategoryTabs" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link active rounded-pill" id="all-tab" data-bs-toggle="tab"
                    ...
                    @foreach ($categories as $index => $category)
                        <li class="nav-item" role="presentation">
                            <button class="nav-link rounded-pill" id="cat-{{ $index }}-tab" data-bs-toggle="tab"
                                data-bs-target="#cat-{{ $index }}" type="button" role="tab"
                                aria-controls="cat-{{ $index }}" aria-selected="false"
                ...
                <div class="products-grid" data-products-container="all">
                    @foreach ($products as $product)
                        <div class="product-item">
                            <x-kartu-produk :id="$product['id']" :name="$product['name']" :price="$product['price']" :image="$product['image']"
                                :badge="null" :oldPrice="$product['oldPrice'] ?? null" :category="$product['category']" :brand="$product['brand'] ?? null" />
                        </div>
                    @endforeach
                </div>
                    ...
                    <div class="pagination-wrapper mt-5" data-pagination="all">
                        ...
                        <div class="text-center mt-3">
                            <small class="text-muted" data-pagination-info>
                                Showing <span class="fw-semibold" data-showing-start>1</span>-<span class="fw-semibold"
                                    data-showing-end>12</span> of <span class="fw-semibold"
                                    data-total-items>{{ count($products) }}</span> products
                            </small>
                        </div>
                    </div>
                @endif
                    ...
                    @if (count($filteredProducts) > 0)
                        <div class="products-grid" data-products-container="cat-{{ $index }}">
                            @foreach ($filteredProducts as $product)
                                <div class="product-item">
                                    <x-kartu-produk :id="$product['id']" :name="$product['name']" :price="$product['price']"
                                        :image="$product['image']" :badge="null" :oldPrice="$product['oldPrice'] ?? null" :category="$product['category']"
                        ...
                        @if (count($filteredProducts) > 12)
                            <div class="pagination-wrapper mt-5" data-pagination="cat-{{ $index }}">
                                ...
                                <div class="text-center mt-3">
                                    <small class="text-muted" data-pagination-info>
                                        Showing <span class="fw-semibold" data-showing-start>1</span>-<span
                                            class="fw-semibold" data-showing-end>12</span> of <span class="fw-semibold"
                                            data-total-items>{{ count($filteredProducts) }}</span> products
                                    </small>
                                </div>
                            </div>
                        @endif
...
<style>
    ...
    .category-pills {
        ...
        width: max-content;
        min-width: 100%;
        /* Agar tetap center jika items sedikit */
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function initPagination(category, items, wrapper) {
            ...
            function generatePageButtons() {
                ...
                const oldButtons = pagination.querySelectorAll(
                    '.page-item:not([data-page-prev]):not([data-page-next])');
            ...
            button.addEventListener('shown.bs.tab', function(e) {
                ...
                if (paginationWrapper) {
                    const productsContainer = document.querySelector(
                        `[data-products-container="${targetId}"]`);
                    const productItems = productsContainer.querySelectorAll('.product-item');
